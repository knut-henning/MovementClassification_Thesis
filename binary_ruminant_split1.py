# -*- coding: utf-8 -*-
""" Binary ruminant classification split1
"""
...
X_train, y_train, X_val, y_val, X_test, y_test = mlp.create_ratio_train_val_test(acc_act, serials, att)
...
mlp.create_hyp_report(
    ...
    X_test, y_test, 
    'gru', gru_config_dict,
    att, 'ruminant', 'split1', show_epochs=True
    )


#%% Best params for GRU
# Reload model and data
ruminant_gru_losses = np.load('BestModels\\ruminant\\GRU\\split1\\val_losses.npy')

ruminant_gru_report = []

for i, model in enumerate(ruminant_gru_losses):
    ruminant_gru_model = keras.models.load_model('BestModels\\ruminant\\GRU\\split1\\model{}'.format(i))
    ruminant_gru_X_test = np.load('BestModels\\ruminant\\GRU\\split1\\model{}_Xtest.npy'.format(i))
    ruminant_gru_y_test = np.load('BestModels\\ruminant\\GRU\\split1\\model{}_ytest.npy'.format(i))
...
# Reload model and data
ruminant_lstm_losses = np.load('BestModels\\ruminant\\LSTM\\split1\\val_losses.npy')

ruminant_lstm_report = []

for i, model in enumerate(ruminant_lstm_losses):
    ruminant_lstm_model = keras.models.load_model('BestModels\\ruminant\\LSTM\\split1\\model{}'.format(i))
    ruminant_lstm_X_test = np.load('BestModels\\ruminant\\LSTM\\split1\\model{}_Xtest.npy'.format(i))
    ruminant_lstm_y_test = np.load('BestModels\\ruminant\\LSTM\\split1\\model{}_ytest.npy'.format(i))
