# -*- coding: utf-8 -*-
""" Multiclass behavior classification split1
"""
...
mlp.create_hyp_report(
    ...
    X_test, y_test, 
    'gru', gru_config_dict,
    att, 'multi', 'split1', show_epochs=True,
    )


#%% Best params for GRU
# Reload model and data
multi_gru_losses = np.load('BestModels\\multi\\GRU\\split1\\val_losses.npy')

multi_gru_report = []

for i, model in enumerate(multi_gru_losses):
    multi_gru_model = keras.models.load_model('BestModels\\multi\\GRU\\split1\\model{}'.format(i))
    multi_gru_X_test = np.load('BestModels\\multi\\GRU\\split1\\model{}_Xtest.npy'.format(i))
    multi_gru_y_test = np.load('BestModels\\multi\\GRU\\split1\\model{}_ytest.npy'.format(i))
...
# Reload model and data
multi_lstm_losses = np.load('BestModels\\multi\\LSTM\\split1\\val_losses.npy')

multi_lstm_report = []

for i, model in enumerate(multi_lstm_losses):
    multi_lstm_model = keras.models.load_model('BestModels\\multi\\LSTM\\split1\\model{}'.format(i))
    multi_lstm_X_test = np.load('BestModels\\multi\\LSTM\\split1\\model{}_Xtest.npy'.format(i))
    multi_lstm_y_test = np.load('BestModels\\multi\\LSTM\\split1\\model{}_ytest.npy'.format(i))
