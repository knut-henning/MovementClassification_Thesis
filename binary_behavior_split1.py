# -*- coding: utf-8 -*-
""" Binary behavior classification split1
"""
...
mlp.create_hyp_report(
    ...
    X_test, y_test, 
    'gru', gru_config_dict,
    att, 'binary', 'split1', show_epochs=True
    )


#%% Best params for GRU
# Reload model and data
binary_gru_losses = np.load('BestModels\\binary\\GRU\\split1\\val_losses.npy')

binary_gru_report = []

for i, model in enumerate(binary_gru_losses):
    binary_gru_model = keras.models.load_model('BestModels\\binary\\GRU\\split1\\model{}'.format(i))
    binary_gru_X_test = np.load('BestModels\\binary\\GRU\\split1\\model{}_Xtest.npy'.format(i))
    binary_gru_y_test = np.load('BestModels\\binary\\GRU\\split1\\model{}_ytest.npy'.format(i))
...
# Reload model and data
binary_lstm_losses = np.load('BestModels\\binary\\LSTM\\split1\\val_losses.npy')

binary_lstm_report = []

for i, model in enumerate(binary_lstm_losses):
    binary_lstm_model = keras.models.load_model('BestModels\\binary\\LSTM\\split1\\model{}'.format(i))
    binary_lstm_X_test = np.load('BestModels\\binary\\LSTM\\split1\\model{}_Xtest.npy'.format(i))
    binary_lstm_y_test = np.load('BestModels\\binary\\LSTM\\split1\\model{}_ytest.npy'.format(i))
